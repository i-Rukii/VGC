...
import { Search, X, Loader2, AlertCircle, Zap, Shield, ChevronRight } from 'lucide-react';
import type { TeamSlot, PokemonData } from '../types';
...
function TypeBadge({ type }: { type: string }) {
  const colors = TYPE_COLORS[type] ?? { bg: 'rgba(255,255,255,0.1)', text: '#fff' };
  return (
...
function FilledSlot({ slot, onRemove }: { slot: TeamSlot; onRemove: () => void }) {
  ...
  return (
    <div className="rounded-sm border border-[#2d2d2d] hover:border-orange-500/30 transition-all duration-200 overflow-hidden group relative"
...
export default function TeamBuilder() {
  const [slots, setSlots] = useState<TeamSlot[]>(INITIAL_SLOTS);
  ...
  const searchPokemon = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`"${query}" not found. Check the spelling.`);
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-white/35 text-sm mt-0.5">Build and analyse your VGC roster</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      <div className="rounded-sm border border-[#2d2d2d] p-4 backdrop-blur-md" style={{ background: '#1a1a1b' }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-[11px] text-white/40 tracking-[0.2em] uppercase font-medium">
            {activeSlot !== null ? `Filling Slot ${String(activeSlot + 1).padStart(2, '0')} — Enter Pokémon name` : 'Select a slot below to begin'}
          </span>
        </div>
        <div className="flex gap-2">
          <div className="flex-1 relative">
            ...
            <input
              ...
              placeholder={activeSlot !== null ? 'e.g. incineroar, flutter-mane, urshifu...' : 'Select a slot first...'}
      ...
      <div className="grid grid-cols-3 gap-4">
        {slots.map((slot) =>
          slot.pokemon ? (
            <FilledSlot key={slot.id} slot={slot} onRemove={() => {
              setSlots(prev => prev.map(s => s.id === slot.id ? { ...s, pokemon: null } : s));
              if (activeSlot === slot.id) setActiveSlot(null);
            }} />
          ) : (
            <EmptySlot key={slot.id} slotNum={slot.id} isActive={activeSlot === slot.id}
